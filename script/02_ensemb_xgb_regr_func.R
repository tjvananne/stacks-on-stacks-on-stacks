...
# 02_ensemb_xgb_regr_function

# this was copied from the classification function, we must remove all traces of classification


# potential issues with xgb regression
# https://github.com/dmlc/xgboost/issues/1581
# should we have scaled the numeric values?


# main() stuff for now, to move quickly on this function def
...
# if we need to jump INTO the function interactively, use these ------
    ...
    # p_tar_var <- "petal_width"  # <-- target of THIS stack, NOT of the overall project
...
sos_xgb_regr <- function(
    p_feats_all,              # <-- no longer using global version, pass the whole df in
    p_stack_y,                # <-- the y's relative to THIS stack, not for the overall project - this is 2 column df, "id" / "whatever y is"
    p_tar_var,                
    p_train_ids,              
    ...
    x_all <- subset(feats_all, (!is.na(value) & (!feature_name %in% c(p_tar_var, p_cheaters))))    
    ...
    for(i in 1:p_cv_folds) {
        ...
        ypred_fold_df <- as.data.frame(ypred_fold)
        names(ypred_fold_df) <- paste0(p_tar_var, "_pred_", p_stack_identifier)
        ...
        if(use_test) {
            ...
            ypred_test_df <- as.data.frame(ypred_test)
            names(ypred_test_df) <- paste0(p_tar_var, "_pred_", p_stack_identifier)
