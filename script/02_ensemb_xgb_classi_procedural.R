...
#' description: classification level1 model on sparse matrices using xgboost
#########################################
...
# think of these like parameters / config settings
    ...
    # x2_cheaters_ <- c("")  # <-- should be based on feature_name within feats_all, NOT based on df_all
    x2_cheaters_ <- unique(feats_all$feature_name[grepl("^species_", feats_all$feature_name)])
...
# DJ, stack that sh**!
    ...
    x2_all <- subset(feats_all, (!is.na(value) & !feature_name %in% c(x2_cheaters_)))
...
# looped manual CV
    ...
    for(i in 1:folds) {
        ...
        dx2_test_ <- xgb.DMatrix(x2_test_sp)                 # <-- this doesn't need to happen in every fold loop
