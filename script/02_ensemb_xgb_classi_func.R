...
# 02_ensemb_xgb_classi_function

# output everything to a file - all output: https://stackoverflow.com/questions/7096989/how-to-save-all-console-output-to-file-in-r


# if we need to jump INTO the function interactively, use these as parameter values:
...
sos_xgb_classi <- function(
    p_feats_all,              # <-- no longer using global version, pass the whole data.frame in
    p_stack_y,                # <-- y's relative to THIS stack, not for whole project; should be 2 col df, c("id", "<target>")
    p_tar_var,                
    p_train_ids,              
    p_test_ids=NULL,          # user doesn't have to supply test ids; useful for imputing missing values or enriching data
    p_xgb_params,             
    p_cv_folds=4,
    p_cv_rounds=3000,
    p_cv_earlystop=15,
    ...
    # printing for remote machine sink log file
    print(paste0("Starting a new sos_xgb_classi where target=", 
                 p_tar_var, " and p_stack_identifier=", p_stack_identifier))
    ...
    for(i in 1:p_cv_folds) {
        ...
        # cross validation - TODO: I want the OPTION to run CV on ALL train to find best nrounds, not just first fold
        #  also, this second idea here in the TODO should cache into a different file marked "_full.rds" or something
